epidemic <- function(n=5500000,ne=10,lambda=0.4/n,pei=1/3,pir=1/5,nd=100) {
  ...
  for (i in 2:nd) { ## loop over days
    ...
    inew[i] <- I[i] - I[i-1] ## new infections of dayi in the whole pop
    inew_lb[i] <- I_lb[i] - I_lb[i-1] ## new infections of dayi in the 10% of the pop with lowest beta
    inew_r[i] <- I_r[i] -I_r[i-1] ## new infections of dayi of a random sample of 0.1% of the population
  }
